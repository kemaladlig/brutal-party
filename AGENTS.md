...
- **Ortak `src/core/` yardımcıları (refactor Faz 1-7, tek kaynak):** Motorlar aşağıdakileri kopyalamaz/yeniden yazmaz, `import` eder:
  ...
  - `fieldKit.js` — **saha zemini/çevre tek kaynağı**: `FIELD_THEMES` (tema paleti + motif/köşe dili) + `FIELD_MOTIFS`, `hashFieldSeed(mode, roundId)`, `paintFieldLayer` (zemin gradyanı + ızgara + çerçeve + motif + seeded dekor + vignette + duvar bandı + yama), `drawField(ctx, arena, { mode, theme, seed, marks, patches, variant })` (offscreen bake + cache + blit), `releaseFieldLayers`. Statik katman bir kez pişirilir, frame başına tek `drawImage` — motor `render()`'ı kendi zemin fill/grid/duvar stroke'unu YAZMAZ, `drawField` çağırır. **Ağa alan eklemez:** dekor `(mode, roundId)`'den hash'lenir, ikisi de world packet'inde zaten vardır; katmanda `Math.random()`/`Date.now()`/`performance.now()` yasaktır. `marks` oyun başına sabit modül fonksiyonudur; veriye bağlı katman `variant` ile anahtarlanır.
...
- **Saha zemini/duvarını motor `render()`'ında yeniden çizme** — `fieldKit.drawField(ctx, arena, { mode, seed })` çağrılır. Zemin dolgusu + ızgara + çerçeve + köşe plakası + duvar konturu yazan ikinci bir yol kalıyorsa bu bir regresyondur (`tests/fieldKit.test.mjs` paket alanlarını, `pxConstants` ölçeklemesini kilitler).
