...
18. **Saha ölçeği tek kaynağı (`src/core/playfield.js`, Eylül 2026):** 15 motorun `resize()` içinde kendi kopyasını taşıdığı kenarlık/arena hesabı tek bir `computePlayfield(w, h, preset)` çağrısına taşındı. Altı kenar biçimi `FIELD_PRESETS` veri tablosunda: `standard` (PONG/ARCHER/BOMB/HEIST/CURVE/NINJA/SNAKE/LASER/COLLAPSE/CLONE), `roomy` (HORDE), `crown`, `flat` (TANKS), `dense` (ZONE), `racing` (RACE).
     ...
     * **`faceMode: 'play'` — oyun içi avatar artık dekor taşımıyor (`compactSilhouette` kaldırıldı):** görsel şişkinliğin kaynağı dekordu, yarıçap değil. Ölçülen çizilen bbox (`render-harness.html?probe=extent&r=16`): gövde **36x37**, `HALO` **36x45** (+%22 dikey), `WINGS` **49x40** (+%36 yatay), `BOLT`/`CROWN`/`NINJA_COWL` 36x37, düşman chaser 32x32. Cihaz profili `HALO` taşıdığı için oyuncunun dikey taşması haloydu; eski çözüm yalnız taşan iki aksesuarı bastırıyordu (`SILHOUETTE_OVERFLOW` + `compactSilhouette`), gerisi (taç, şapka, gözlük, desen) sahada kalıyordu. Artık **tek kip** var: `drawGameAvatar` her çağrıda `faceMode: 'play'` ister, `drawBrutalAvatar` bu kipte aksesuarı/deseni `NONE`/`SOLID`'e sabitler, gözleri büyütür (0.24r → 0.30r) ve disk İÇİ hacim ekler (sol üst ışık + sağ alt gölge; gradient'ler `ctx -> yarıçap` WeakMap'inde cache'lenir, karede yeni gradient üretilmez, maliyet iki `fillRect`). Menü/lobi/kumanda önizlemesi `drawBrutalAvatar`'ın doğrudan çağıranlarıdır ve varsayılan `full` kipte **değişmedi** — kimlik kaybolmadı, sadece sahadan çıktı. Canlılık: koltuk fazına kaydırılmış göz kırpma (`blinkState`, 2.6s periyot / 110ms kapalı; view `now`'unu motorun `this.lastTime`'ı besler) ve gövdeden bağımsız bakış (`lookAngle`, 0.22 rad kırpma; ARCHER nişan sallanması, BOMB/HEIST koşu yönü, HORDE `targetAngle` — HORDE world packet'ine `lookAngle` girdi, `accessory`/`pattern` alanları çıktı). Gövde deformasyonu (breathing/squash) BİLEREK yok: %4 bile olsa yuvarlaklığı bozuyordu. Ölçülen sonuç: her ifade + her bakış açısında bbox **36x37**. `tests/avatarInGame.test.mjs` (eski `avatarSilhouette.test.mjs`) üç katmandan gelen dekoru (opts, player alanı, sahte localStorage'daki cihaz profili) çizim imzasıyla kilitliyor, bbox'u sınırlandırıyor ve menü kipinin dekoru çizmeye devam ettiğini doğruluyor.
